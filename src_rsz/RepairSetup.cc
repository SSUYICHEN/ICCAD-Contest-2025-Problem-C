...
void RepairSetup::reportCellsNegPath(double percentage)
{
  if (percentage < 0)
    percentage = 0;
  ...
  vector<pair<Vertex*, Slack>> ends;
  for (Vertex* end : *endpoints) {
    const Slack end_slack = sta_->vertexSlack(end, max_);
    if (end_slack < 0)
      ends.emplace_back(end, end_slack);
  }
  if (ends.empty()) 
    return;
    
  std::stable_sort(ends.begin(),
                   ends.end(),
  ...
  int end_index = 0;
  int max_end_count = max(1, int(0.01 * percentage * ends.size()));

  // Ensure that max cap and max fanout violations don't get worse
  sta_->checkCapacitanceLimitPreamble();
  sta_->checkFanoutLimitPreamble();
  
  std::unordered_map<std::string, Delay> load_delays;
  for (const auto& end_original_slack : ends) {
    ...
    for (int i = expanded.startIndex(); i < expanded.size(); i++) {
      ...
      if (i > 0 && network_->isDriver(path_pin)
        ...
        for (char& c : (std::string)path_vertex->name(network_)) {
          if (c == '/')
            break;
          cell_name += c;
        }*/
        load_delays[path_vertex->name(network_)] += load_delay;
      }
    }
  }  
  
  vector<pair<std::string, Delay>> load_delays_list;
  ...
  sort(
      ...
      [](pair<std::string, Delay> pair1, pair<std::string, Delay> pair2) {
        return pair1.second > pair2.second;
  });
  for (auto& item : load_delays_list) {
...
/*
  ...
  int end_index = 0;
  int max_end_count = max(1, int(percentage / 100 * ends.size()));

  // Ensure that max cap and max fanout violations don't get worse
  sta_->checkCapacitanceLimitPreamble();
  sta_->checkFanoutLimitPreamble();
  
  min_viol_ = -ends.back().second;
  max_viol_ = -ends.front().second;
  std::cout << "min_viol: " << min_viol_ << std::endl;
  std::cout << "max_viol: " << max_viol_ << std::endl;
  ...
  for (const auto& end_original_slack : ends) {
    ...
    Path* end_path = sta_->vertexWorstSlackPath(end, max_);
    PathExpanded expanded(end_path, sta_);
    vector<pair<std::string, Delay>> load_delays;
    const DcalcAnalysisPt* dcalc_ap = end_path->dcalcAnalysisPt(sta_);
    const int lib_ap = dcalc_ap->libertyIndex();
    ...
    for (int i = expanded.startIndex(); i < expanded.size(); i++) {
      ...
      Vertex* path_vertex = path->vertex(sta_);
      const Pin* path_pin = path->pin(sta_);
      std::cout<<path_vertex->name(network_)<<" ";
      if (i > 0 && network_->isDriver(path_pin)
          && !network_->isTopLevelPort(path_pin)) {
        const TimingArc* prev_arc = path->prevArc(sta_);
        const TimingArc* corner_arc = prev_arc->cornerArc(lib_ap);
        ...
        const Delay load_delay
            = graph_->arcDelay(prev_edge, prev_arc, dcalc_ap->index())
              // Remove intrinsic delay to find load dependent delay.
              - corner_arc->intrinsicDelay();
        load_delays.emplace_back(path_vertex->name(network_), load_delay);
      }
    }std::cout<<std::endl;
    sort(
        load_delays.begin(),
